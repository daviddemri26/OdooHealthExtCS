...
When an authorized user opens a matching Odoo subscription form whose visible status badge is **In Progress** or **Paused** and enables a feature, the extension reads only the fields needed to show account health and the linked customer's industry. It processes tag identifiers and names, the subscription's linked partner identifier and display name, the current industry, and the available industry choices.
...
When Share Links is enabled, the extension checks only the current quotation's server `state` and `subscription_state`. It can show the shortcut on draft or sent Renewal Quotations opened under Subscriptions or through Odoo's nested technical `sale.order` routes, and on draft or sent quotations opened under Sales, including Success Packs. After an explicit click, it asks Odoo's native Share wizard to create or reuse the current `sale.order` access link and writes that validated URL to the clipboard. It does not send a Share email, read the clipboard, or expose the token in a notification.
...
Browser synchronized storage contains only versioned extension preferences: master enablement, feature enablement, Share Link target enablement, subscription-list preview enablement, per-feature success-confirmation preferences, renewal discount presets, and appearance. Browser local storage may contain a sanitized general connection code and timestamp. The connected-user label exists only in volatile memory while the matching Odoo tab remains open. A generated Share URL is handed directly to the browser clipboard after the click and is not stored by the extension. The extension does not persist Share URLs, Odoo records, customer data, names, logins, email addresses, cookies, passwords, CSRF values, user identifiers, session identifiers, credentials, request payload histories, or raw server stack traces.
...
The extension requests `storage` for settings and sanitized compatibility status and `clipboardWrite` for explicit user-triggered copy actions. It does not request clipboard-read access and never inspects existing clipboard content. Its isolated interface script and page-context bridge are both limited to `https://www.odoo.com/odoo*`. It does not request browsing history, tabs, cookies, identity, downloads, geolocation, camera, microphone, or broad host access.
