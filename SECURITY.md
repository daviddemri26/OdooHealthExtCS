...
The extension uses the active same-origin Odoo session and two narrowly matched content scripts: an isolated interface and a `MAIN`-world RPC bridge. The bridge must reject every model, method, field, domain, operation, and write shape outside the reviewed Health, Industry, Renewals, and Share Links allow-lists before fetch, and it must return only sanitized fields and failures. Share URLs must be exact same-origin `sale.order` links bound to the current record and must never enter logs or storage. Clipboard access remains write-only and user-triggered. The extension must not add credential collection, external telemetry, remote code execution, cookie access, clipboard reads, or broad host permissions. Store credentials belong only in protected GitHub environments and must be rotated after suspected exposure.
